...
function createDummyInvoice(invoiceId) {
  return {
    ...
    payload_json: JSON.stringify({
      ...
      billing: {
        name: 'BORI',
        email: 'test121232133@naver.com',
        address: '서울시 강남구 테헤란로 123'
      },
      shipping: {
        name: 'BORI',
        email: 'test12123423143123@naver.com',
        address: '서울시 강남구 테헤란로 123'
      },
      amounts: {
        net: 1500000,
...
function renderInvoiceDetail(invoice) {
    ...
    <div class="invoice-detail-header">
      <div class="invoice-header-left">
        ...
        <div class="invoice-header-meta">
          ...
          <div>ISSUE DATE : ${escapeHtml(issuedDate)}</div>
        </div>
      </div>
      <div class="invoice-header-right">
        <svg class="invoice-brand-logo" viewBox="0 0 155 29" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  // 768px 이하일 때 로고를 invoice-header-left 오른쪽으로 이동
  function moveLogoToHeaderLeft() {
    ...
    if (!headerLeft || !headerRight || !logo) {
      return;
    }
    
    const windowWidth = window.innerWidth;
    if (windowWidth <= 768) {
      // 로고가 아직 header-right에 있으면 header-left로 이동
      if (logo.parentElement === headerRight) {
        headerLeft.appendChild(logo);
      }
    } else {
      // 768px 초과일 때는 header-right로 다시 이동
      if (logo.parentElement === headerLeft) {
  ...
  // 초기 로고 위치 설정
  moveLogoToHeaderLeft();
  
  // 화면 크기 변경 시 로고 위치 재조정
  window.addEventListener('resize', () => {
    moveLogoToHeaderLeft();
  });
  
  // 보증서 버튼 위치 재계산 함수
  function recalculateWarrantyButtonPosition() {
    ...
    const mainWrapper = documentContainer?.closest('.invoice-main-wrapper');
    ...
    if (!warrantyButtonsColumn || !documentContainer || !mainWrapper || !descriptionSection || !table || items.length === 0) {
      return;
    }
    
    const windowWidth = window.innerWidth;
    const isTabletOrBelow = windowWidth <= 1439;
    
    if (isTabletOrBelow) {
      // 769px ~ 1439px: 문서 컨테이너의 오른쪽 경계 계산
      const containerRect = documentContainer.getBoundingClientRect();
      const wrapperRect = mainWrapper.getBoundingClientRect();
      
      // 문서 컨테이너의 오른쪽 경계가 wrapper의 오른쪽 경계로부터 얼마나 떨어져 있는지
      const containerRightFromWrapper = wrapperRect.right - containerRect.right;
      
      // 버튼의 절반이 밖으로 나가도록 right 값 계산
      // 버튼의 대략적인 너비 (실제 버튼 너비 측정)
      const buttonWidth = warrantyButtonsColumn.offsetWidth || 120; // 기본값 120px
      const buttonHalfWidth = buttonWidth / 2;
      
      // 문서 컨테이너의 오른쪽 경계에서 버튼의 절반만큼 밖으로 나가도록
      const rightValue = containerRightFromWrapper - buttonHalfWidth;
      
      warrantyButtonsColumn.style.right = `${rightValue}px`;
    }
  ...
  setTimeout(() => {
    ...
    if (warrantyButtonsColumn && documentContainer && descriptionSection && table && items.length > 0) {
      ...
      // 초기 위치 계산
      recalculateWarrantyButtonPosition();
      
      // ResizeObserver로 화면 크기 변경 감지 및 재계산
      const resizeObserver = new ResizeObserver(() => {
        recalculateWarrantyButtonPosition();
      });
      
      // 문서 컨테이너, 테이블, 버튼 컬럼, 메인 래퍼를 관찰
      resizeObserver.observe(documentContainer);
      resizeObserver.observe(table);
      resizeObserver.observe(warrantyButtonsColumn);
      if (mainWrapper) {
        resizeObserver.observe(mainWrapper);
      }
      
      // window resize 이벤트도 추가 (추가 안전장치)
      let resizeTimeout;
      window.addEventListener('resize', () => {
