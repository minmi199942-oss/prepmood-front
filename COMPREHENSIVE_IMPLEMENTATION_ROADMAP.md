...
- ❌ `warranties.status` 컬럼 (issued/active/revoked 등)
- ❌ `warranties.owner_user_id` 컬럼
- ❌ `warranties.source_order_item_unit_id` 컬럼
- ❌ `warranties.activated_at`, `warranties.revoked_at` 컬럼
- ❌ `shipments` 테이블
- ❌ `shipment_units` 테이블
- ❌ `warranty_events` 테이블
...
### Phase 0: 옵션 API 개선 (즉시 개선 가능, 충돌 없음)
...
### Phase 1: 색상 데이터 소스 개선 (즉시 개선 가능, 충돌 없음)
...
### Phase 10: 비회원 주문 조회 API 구현

**목적**: 비회원이 주문 조회 (guest_order_access_token 사용)
...
**API 엔드포인트**: `GET /api/guest/orders/:orderId`

**인증**: `guest_order_access_token` (쿠키 또는 헤더)
...
### Phase 11: orders.status 집계 함수 구현
...
### 즉시 실행 가능 (의존성 없음)
1. **Phase 0**: 옵션 API 개선
2. **Phase 1**: 색상 데이터 소스 개선

### Phase 2 완료 후 가능
3. **Phase 3**: processPaidOrder() 업데이트
4. **Phase 5**: 보증서 활성화 API
5. **Phase 7**: QR 스캔 로직 수정
6. **Phase 12**: 관리자 페이지 개선

### Phase 2 + Phase 3 완료 후 가능
7. **Phase 4**: 선예약형 재고 관리 (선택적, 현재 방식도 동작)

### Phase 2 + Phase 5 완료 후 가능
8. **Phase 6**: Claim API
9. **Phase 8**: 양도 시스템
10. **Phase 9**: 환불 처리 API

### Phase 2 + Phase 6 완료 후 가능
11. **Phase 10**: 비회원 주문 조회 API
12. **Phase 14**: 프론트엔드 사용자 페이지

### Phase 2 + Phase 12 완료 후 가능
13. **Phase 12**: 배송/송장 관리 API

### Phase 2 완료 후 가능 (독립적)
14. **Phase 11**: orders.status 집계 함수
...
### QR 코드 흐름
- [ ] Paid 처리 시 warranties 생성 (`status = 'issued'` 또는 `'issued_unassigned'`)
- [ ] QR 스캔 시 warranty 조회만 수행 (생성 제거)
- [ ] QR 스캔 시 revoked 상태 보증서 접근 거부
- [ ] 관리자 페이지에서 QR 코드 다운로드 가능

### 디지털 보증서 흐름
- [ ] 보증서 활성화 API 정상 동작 (인보이스 연동 확인 포함)
- [ ] 활성화된 보증서만 양도 가능
- [ ] 양도 시 소유자만 변경, 상태는 active 유지
- [ ] 환불 시 warranties.status → revoked 전이
...
**즉시 시작 가능한 작업**:
1. Phase 0: 옵션 API 개선 (2-3시간)
2. Phase 1: 색상 데이터 소스 개선 (1-2시간)

**Phase 2 완료 후 시작**:
3. Phase 3: processPaidOrder() 업데이트 (2-3시간)
4. Phase 5: 보증서 활성화 API (3-4시간)
...
**작성일**: 2026-01-11  
**최종 수정일**: 2026-01-11  
**기준 문서**: `SYSTEM_FLOW_DETAILED.md`, `FINAL_EXECUTION_SPEC_REVIEW.md`, `ADMIN_QR_WARRANTY_INVOICE_CONSISTENCY_CHECK.md`
