...
**목적**: JavaScript 문법 오류 확인 (괄호, 세미콜론, 변수명 등)
...
**설명**:
- `node -c`는 파일을 실행하지 않고 문법만 체크합니다
- 문법 오류가 있으면 에러 메시지 출력
- 문법 오류가 없으면 아무것도 출력하지 않음

**참고**: 
- 이미 자동 배포가 완료되었다면 문법 오류는 없습니다 (문법 오류가 있으면 배포 시 에러 발생)
- 이 단계는 선택사항이며, 실제 테스트 주문으로 확인하는 것이 더 중요합니다
